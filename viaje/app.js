    ...
    constructor() {
        ...
        this.defaultData = {
            ...
            certificateData: {
                ...
                confettiShown: false // <-- Para controlar si ya se mostró el confeti
            },
            studentName: '',
    ...
    lessons: {
        intro: {
            ...
            content: `
                <div class="lesson-intro">
                    <p><strong>¡Bienvenida, bienvenido!</strong> Antes de comenzar, por favor indícanos tu nombre completo para personalizar tu certificado de finalización.</p>
                    
                    <div style="background: #f0f9ff; border: 2px solid #bae6fd; border-radius: 12px; padding: 1.5rem; margin: 2rem 0; text-align: center;">
                        <label for="studentName" style="display: block; font-weight: 600; margin-bottom: 0.5rem; color: #0c4a6e;">
                            ¿Cuál es tu nombre completo?
                        </label>
                        <input type="text" id="studentName" placeholder="Ej: María González Pérez" style="width: 100%; max-width: 400px; padding: 0.75rem; border: 2px solid #bae6fd; border-radius: 8px; font-size: 1rem; margin-bottom: 1rem;">
                        <button class="btn btn-primary" id="saveStudentName">
                            <i class="fas fa-user-check"></i> Guardar y Continuar
                        </button>
                    </div>

                    <p>En el corazón de cada educador reside una pasión por encender la curiosidad y guiar el aprendizaje. Sin embargo, las realidades del día a día —la planificación interminable, la carga administrativa y la necesidad de atender a una diversidad de estudiantes— pueden opacar esa llama.</p>
        ...
        '1.1': {
            ...
            activity: {
                ...
                description: 'Para tener una primera experiencia tangible y positiva, te invitamos a abrir una de estas herramientas (como ChatGPT, Gemini o Claude) y probar con una pregunta sencilla. Esto te permitirá ver la magia en acción.',
                ...
                <div class="certificate-content">
                    ...
                    <div style="text-align: center; margin: 2rem 0;">
                        <a href="BibliotecaRapidaPromptsEducadores.pdf" download class="btn btn-primary" style="font-size: 1.1rem; padding: 1rem 2rem;">
                            <i class="fas fa-download"></i> Descargar PDF de Prompts
                        </a>
...
if (submitCommentBtn) {
    submitCommentBtn.addEventListener('click', function() {
        ...
        if (comment) {
            ...
            // Mostrar botón "Siguiente" si existe
            const nextLessonId = storageManager.getNextLesson(lessonId);
            if (nextLessonId) {
...
class Navigation {
    static init() {
        ...
        if (shareCertificateBtn) {
            shareCertificateBtn.addEventListener('click', () => {
                if (navigator.share) {
                    navigator.share({
                        title: '¡He completado el curso de Educador Potenciado por IA!',
                        text: `¡Mira mi certificado! Nombre: ${studentName}`,
                        url: window.location.href
                    });
                } else {
                    alert('¡Comparte tu logro en redes sociales!');
                }
            });
        }
    ...
    static showCertificate() {
        ...
        document.getElementById('studentNameInBtn').textContent = studentName;

        // Mostrar certificado visual
        certificatePreview.style.display = 'block';

        // Disparar confeti SOLO la primera vez que se completa el curso
...
function generatePDF(name, date, certId) {
    ...
    doc.text("CERTIFICADO", 148, 25, { align: 'center' });

    doc.setFontSize(14);
    doc.setFont('times', 'italic');
    doc.text("de Finalización y Excelencia Académica", 148, 35, { align: 'center' });

    // Contenido principal
    doc.setTextColor(0, 51, 102);
    doc.setFontSize(18);
    doc.setFont('times', 'normal');
    doc.text("Este certificado atesta que", 148, 70, { align: 'center' });
    ...
    doc.setFontSize(16);
    doc.setFont('times', 'normal');
    doc.text("ha completado con éxito el curso", 148, 120, { align: 'center' });
    doc.setFont('times', 'bold');
    doc.text("“Capacitación en IA: Herramientas esenciales para docentes”", 148, 130, { align: 'center' });
    doc.setFont('times', 'normal');
    doc.text("demostrando un dominio de los principios y la aplicación práctica", 148, 140, { align: 'center' });
    doc.text("de la inteligencia artificial para optimizar la enseñanza.", 148, 150, { align: 'center' });

    // Fecha de finalización
    doc.setFontSize(16);
    doc.text(`Finalizado el ${date.toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}.`, 148, 170, { align: 'center' });

    // Firma
    doc.setFontSize(14);
    doc.setFont('times', 'bold');
    doc.text("Dirección General de Capacitación IA", 148, 195, { align: 'center' });

    // Pie de página
    doc.setFontSize(10);
    doc.setFont('times', 'italic');
    doc.setTextColor(102, 102, 102);
    doc.text(`Documento oficial • ID: IA-DOC-${certId} • Fecha de emisión: ${date.toLocaleDateString('es-ES')}`, 148, 270, { align: 'center' });

    // Descargar
    const filename = `Certificado_IA_Docentes_${name.replace(/\s+/g, '_')}.pdf`;
    doc.save(filename);
    alert(`🎉 ¡Certificado oficial descargado!\nGuardado como: ${filename}`);
}
